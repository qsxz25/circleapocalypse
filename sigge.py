...
enemy_r = 75
enemysource_rec = Rectangle(0,0,32,32)
...
def draw_game():
    ...
    global x, y, r, target_x, target_y, target_r, score, game_state, enemy_directions, last_direction_change, enemies,playerdesc_rec,playersource_rec
    ...
    # Draw score
    draw_text(f"score = {score}",00,20,32,WHITE)
    ...
    for i in range(len(enemies)):
        ...
        enemydesc_rec.x = enemy_x
        enemydesc_rec.y = enemy_y
    ...
    for enemy_x, enemy_y, enemy_r in enemies:
        ...
        if check_collision_circle_rec((x, y), r,enemy_rec):
            print("YOU DIED")
    ...
    # Simple game message
    draw_text("Game is running! Press P for menu", 00, 00, 20, WHITE)
...
def draw_settings():
    ...
    for i, settings in enumerate(settings_options):
        draw_text(settings, 100, 200 + i * 40, 24, WHITE)
...
def reset_game():
    ...
    global x, y, r, score, target_x, target_y, enemies, enemy_directions, last_direction_change, direction,playerdesc_rec
   
    print(f"Resetting game - Player position before: x={x}, y={y}")  # Debug line
    print(f"Enemies before reset: {enemies[:2]}")  # Show first 2 enemies
...
def handle_game_input():
    ...
    if pyray.is_key_pressed(pyray.KEY_L):
